...
def graph_statistics(inpath,pattern):
    ...
    for l in [glob.glob(os.path.join(inpath, '*.nde')), glob.glob(os.path.join(inpath, '*.hist'))]:
        for filename in l:
            ...
            graph_dataset.append([keyname,SHORT_NAMES[keyname], SHORT_NAMES[keyname]+"ss",TYPES[keyname], graph.numberOfNodes(), graph.numberOfEdges(),round(graph.numberOfEdges()/graph.numberOfNodes(),2),statistics.median(degrees),max(degrees),"\\true{2pt}" if "random" in TYPES[keyname] else "\\false{2pt}", "\\textbf{"+ str(lll_threshold) +"}" if const >= 1 else lll_threshold]);
...
def ap1AlgoBarPlots(inpath,pattern):
    ...
    for g in grafi.unique():
        ...
        for var in ['gamma','gamma-unhappy', 'nash-unhappy','avgpayoff','time', 'iterations']:
            ...
            for p in fig.patches:
                # $$(used="+str(n_used[cou])+")$$
                fig.annotate("$$(k="+str(k_values[cou])+")$$$$"+format(p.get_height(), '.2f')+"$$", (p.get_x() + p.get_width() / 2., p.get_height()+maxH/8.), ha = 'center', va = 'center', xytext = (0, 10), textcoords = 'offset points',color="black",fontsize='x-small')
                cou+=1    
        ...
        for a in algos:
            if head==True:
                ...
                table.append(("\\textsc{"+var+"}",a, 
                              str(best[var].count(a))+" ("+str(round(100*best[var].count(a)/len(best[var]),1))+" \\%)", 
                              str(second[var].count(a))+" ("+str(100*round(second[var].count(a)/len(second[var]),1))+" \\%)"))
                head=False
            else:
                table.append(("",a, 
                              str(best[var].count(a))+" ("+str(round(100*best[var].count(a)/len(best[var]),1))+" \\%)", 
    ...
    text+=tabulate(table,tablefmt='latex_raw',headers=["\\textbf{Metric}","\\textbf{algorithm}","\\textbf{best}","\\textbf{2nd}", "\\textbf{total}"])
...
def multiAlgoBarPlots(inpath,pattern):
    ...
    # if len(grafi.unique())>1:
    #     raise Exception('pattern contains different graphs data')
    metric_to_latex = {'gamma': "\\ensuremath{\gamma(G,c)}", 'gamma-unhappy': "GU(G,c)", 'nash-unhappy': "\\ensuremath{U(G,c)}",
    ...
    for g in grafi.unique():
        ...
        # paletteset=sns.color_palette("dark").as_hex()
        # print(palet)
        paletteset = {metric_to_latex['gamma']: "#65312e",metric_to_latex['gamma-unhappy']: "#d95232", metric_to_latex['nash-unhappy']: "#f29541", metric_to_latex['avgpayoff']: "#e3db9a", metric_to_latex['time']: "#0B5650"}
        ...
        for var in ['gamma', 'nash-unhappy','avgpayoff','time']:
            ...
            for p in fig.patches:
                # $$(used="+str(n_used[cou])+")$$
                fig.annotate("$$"+format(p.get_height(), '.2f')+"$$", (p.get_x() + p.get_width() / 2., p.get_height()), ha = 'center', va = 'center', xytext = (0, 10), textcoords = 'offset points',color="black",fontsize='x-small')
                cou+=1
            if var=='nash-unhappy': 
            ...
            if  var=='avgpayoff':
                ...
                fig.text(.5,.9,'\\ensuremath{\overline{P}(G,c)}',horizontalalignment='center',transform=fig.transAxes)
    ...
    algo_to_latex = {'\\textsc{llg}': "\\LLLshort", '\\textsc{ap1}': "\\apshort", '\\textsc{rnd}': "\\rndshort", '\\textsc{mbr}' : "\\greshort"}
        ...
        for a in algos:
            if head==True:
                ...
                table.append(("\\hline\\multirow{4}{*}{\\textsc{"+var_to_latex[var]+"}}",algo_to_latex[a], 
                              str(best[var].count(a))+" ("+str(round(100*best[var].count(a)/len(best[var]),0))+" \\%)", 
                              str(second[var].count(a))+" ("+str(100*round(second[var].count(a)/len(second[var]),0))+" \\%)", 
                              str(third[var].count(a))+" ("+str(round(100*third[var].count(a)/len(third[var]),0))+" \\%)", 
                              str(worst[var].count(a))+" ("+str(round(100*worst[var].count(a)/len(worst[var]),0))+" \\%)", 
                ...
                table.append(("",algo_to_latex[a], 
                              ...
                              str(len(worst[var]))+" (100 \\%)"))
    from tabulate import tabulate
    text+=tabulate(table,tablefmt='latex_raw',headers=["\\textbf{metric}","\\textbf{algorithm}","\\textbf{best}","\\textbf{2nd}", "\\textbf{3rd}","\\textbf{worst}", "\\textbf{total}"])
...
def multiKLinePlot(inpath,pattern,sampling=True,sampling_interval=0.1):
    ...
    if sampling==True:
        ...
        for graph_instance in grafi.unique():
            ...
            for kval in k_values.unique():
                max_iters = max(resulting_df[(resulting_df['graph_name']==graph_instance) & (resulting_df['k']==kval)]['iterations'].unique());
                sampling_points = round (max_iters * sampling_interval)    
                resulting_frames.append(resulting_df[(resulting_df['k']==kval) & (resulting_df['graph_name']==graph_instance) & ((resulting_df['iterations']) % sampling_points == 0)])
                resulting_frames.append(resulting_df[(resulting_df['k']==kval) & (resulting_df['graph_name']==graph_instance) & ((resulting_df['iterations']) == max_iters)])
    ...
    for graph_instance in grafi.unique():
        ...
        for var in ['gamma','gammafocus','gamma-unhappy', 'nash-unhappy','avgpayoff']:
                ...
                if graph_instance == 'twitter.hist':
                    ...
                    with open('twitter.hist_339154_12_02_2022_18_21_31_525560.out', 'r') as f:
                        for l in f.readlines():
                            ...
                            lll.append(x)
                    plt.plot(lll, 'b', linewidth=3.0)
                g = sns.lineplot(x='iterations',y='gamma',data=resulting_df[resulting_df['graph_name']==graph_instance],hue_order=["$%s$" % x for x in k_s],alpha=0.9,linewidth=4,markers=True, color='#D32F2F')
            else:
                g = sns.lineplot(x='iterations',y=var,data=resulting_df[resulting_df['graph_name']==graph_instance],hue_order=["$%s$" % x for x in k_s],alpha=0.9,linewidth=4,markers=True, color='#D32F2F')
            ...
            plt.legend(loc="lower left",mode="expand", bbox_to_anchor=(0,1.02,1,0.2),fancybox=True, shadow=True, ncol=4)
            ...
            stringoutput = var+"_multi_k_"+str(min_k)+"_"+str(max_k)+"_"+graph_instance.partition(".hist")[0]+".pdf"
...
#                fig.annotate("\\textbf{$$(k="+str(k_values[epsilons[cou]])+")$$$$"+format(p.get_height(), '.2f')+"$$}", (p.get_x() + p.get_width() / 2., p.get_height()+maxH/8.), ha = 'center', va = 'center', xytext = (0, 10), textcoords = 'offset points',color="black",fontsize='x-small')
                fig.annotate("$$(k="+str(k_values[epsilons[cou]])+")$$$$(used="+str(used_values[epsilons[cou]])+")$$$$"+format(p.get_height(), '.2f')+"$$", (p.get_x() + p.get_width() / 2., p.get_height()+maxH/8.), ha = 'center', va = 'center', xytext = (0, 10), textcoords = 'offset points',color="black",fontsize='xx-small')
                cou+=1
...
def ap3_vs_allBarPlots(inpath,pattern):
    ...
    for g in grafi.unique():
        ...
        for var in ['gamma', 'nash-unhappy','avgpayoff','time']:
            ...
            fig=sns.barplot(x="algo", y="value",palette=paletteset,
                                hue="metrics", data=newdf.replace(metric_to_latex))
            plt.legend(loc="lower left",mode="expand", bbox_to_anchor=(0,1.02,1,0.2),fontsize='24',handlelength=1,fancybox=True, shadow=True,ncol=4)
            plt.legend([],[],frameon=False)
            ...
            for p in fig.patches:
                ...
                cou += 1
            fig.text(0.1, .9, '$\\epsilon='+str(epsilon)+'$', horizontalalignment='center', transform=fig.transAxes, fontsize='xx-small')
            fig.text(0.9, .9, '$k='+str(k_values[0])+'$', horizontalalignment='center', transform=fig.transAxes, fontsize='xx-small')
...
def multiAlgomultiKBarPlots(inpath,pattern):
    ...
    for g in grafi.unique():
        ...
        for var in ['gamma', 'nash-unhappy', 'avgpayoff', 'time']:
            ...
            plt.legend(loc="lower left", bbox_to_anchor=(0,1.04, 1, 1), fontsize='18', ncol= len(newdf['k'].unique()), mode="expand", handlelength=1,fancybox=True, shadow=True)
        ...
        for a in algos:
            if head==True:
                ...
                table.append(("\\hline\\multirow{4}{*}{\\textsc{"+var_to_latex[var]+"}}",algo_to_latex[a], 
                              ...
                              str(second[var].count(a))+" ("+str(round(100*second[var].count(a)/210,1))+" \\%)", 
                              str(third[var].count(a))+" ("+str(round(100*third[var].count(a)/210,1))+" \\%)", 
                              str(worst[var].count(a))+" ("+str(round(100*worst[var].count(a)/210,1))+" \\%)", 
                              str(len(worst[var]))+" (100 \\%)"))
                head=False
            else:
                table.append(("",algo_to_latex[a], 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--p",metavar="PATTERN or FILE", required=True, default="", help="pattern to csv files (or single file)")
    parser.add_argument("--t",metavar="PLOT_TYPE", required=True, default=0, help="plot_Type [0: ap3 various eps - 1: dynamics multi_k - 2: multi_algo single_k - 3: graph_analysis - 4: ap3_vs_ap1 - 5: multiAlgo-multiK - 6: graph_statistics (includes LLL) - 7: Ap1 based]")
    parser.add_argument("--n",metavar="NEW_GRAPH_NAME", required=False, default='', help="Name of the file which will contain the cleared graph by nk")
