...
if __name__ == "__main__":
    ...
    parser.add_argument("--k", metavar="NUM_KOLORS", required=False, default=3,
                        help="Number of colors to use, default=3; -1 if multi-k")
    parser.add_argument("--i", metavar="NUM_DYN_ITERS", required=False, default=1,
                        help="Number of iterations to use for dynamics estimation, default=1")
    parser.add_argument("--p", metavar="OUTPUT_FILE_PREFIX", required=False, default='',
                        help="String to preprend in output filename, default=''")
    parser.add_argument("--s", metavar="SEED", required=False, default=2022,
                        help="Seed to replicate random choices, default=2022")
    ...
    for KOLORS in IN_KOLORS:
        ...
        if int(args.a) == CODE['RND']:  # random coloring
            ...
            for _ in range(NOISE_ITERS):
                ...
                auxiliary.dumpOneShotData(graph_name, col_graph, elapsed, ITERATIONS, str(KOLORS), NAME[int(args.a)],
            ...
            if col_graph.colored() == False:
                raise Exception('Non fully colored graph')
            auxiliary.dumpOneShotData(graph_name, col_graph, elapsed, ITERATIONS, str(KOLORS), NAME[int(args.a)],
                                      NULL_EPSILON, gammavalue, isnashboolean, fractionvalue_nash, isgammanashboolean,
                                      fractionvalue_gamma_nash)
            ...
            auxiliary.dumpOneShotData(args.p + str(args.s) + '_' + str(EPSILON) + '_' + graph_name, col_graph, elapsed,
                                      ITERATIONS, str(kprime),
                                      NAME[int(args.a)], EPSILON, gammavalue, isnashboolean, fractionvalue_nash,
                                      isgammanashboolean, fractionvalue_gamma_nash)
            ...
            auxiliary.dumpOneShotData(graph_name, col_graph, elapsed, itrs, str(KOLORS), NAME[int(args.a)],
                                      NULL_EPSILON, gammavalue, isnashboolean, fractionvalue_nash, isgammanashboolean,
                                      fractionvalue_gamma_nash)
            ...
            with open(statsfile, 'a', newline='') as csvfile:
                ...
                for iteration in range(NOISE_ITERS):  # to reduce noise due to randomness
                    ...
                    if col_graph.numberOfUsedColors() > KOLORS:
                        raise Exception('Anomalous usage of KOLORS')
                    gammavalue, isnashboolean, fractionvalue_nash, isgammanashboolean, fractionvalue_gamma_nash = col_graph.nashStatus()
                    auxiliary.dumpMultiData(csvfile, graph_name, col_graph, elapsed, ITERATIONS, str(KOLORS),
                                            NAME[CODE['RND']], NULL_EPSILON, avgdeg, maxdeg, isnashboolean,
                                            isgammanashboolean, gammavalue, fractionvalue_nash,
                                            fractionvalue_gamma_nash)
                ...
                gammavalue, isnashboolean, fractionvalue_nash, isgammanashboolean, fractionvalue_gamma_nash = col_graph.nashStatus()
                auxiliary.dumpMultiData(csvfile, graph_name, col_graph, elapsed, ITERATIONS, str(KOLORS),
                                        NAME[CODE['AP1']], NULL_EPSILON, avgdeg, maxdeg, isnashboolean,
                                        isgammanashboolean, gammavalue, fractionvalue_nash, fractionvalue_gamma_nash)
                ...
                for iteration in range(NOISE_ITERS):  # to reduce noise due to randomness
                    ...
                    auxiliary.dumpMultiData(csvfile, graph_name, col_graph, elapsed, itrs, str(KOLORS),
                                            NAME[CODE['LLL']], NULL_EPSILON, avgdeg, maxdeg, isnashboolean,
                                            isgammanashboolean, gammavalue, fractionvalue_nash,
                                            fractionvalue_gamma_nash)
                ...
                for iteration in range(NOISE_ITERS):
                    ...
                    if col_graph.numberOfUsedColors() > KOLORS:
                        raise Exception('Anomalous usage of KOLORS')
                    auxiliary.dumpMultiData(csvfile, graph_name, col_graph, elapsed, itrs, str(KOLORS),
                                            NAME[CODE['ADY']], NULL_EPSILON, avgdeg, maxdeg, isnashboolean,
                                            isgammanashboolean, gammavalue, fractionvalue_nash,
                                            fractionvalue_gamma_nash)
            ...
            if NAME[int(args.a)] != NAME[CODE['DYN']]:
                raise Exception('wrong algo name')
            statsfile = args.p + str(args.s) + '_' + graph_name + "_" + NAME[CODE['DYN']] + "_" + str(
            ...
            with open(statsfile, 'a', newline='') as csvfile:
                ...
                auxiliary.dumpMultiData(csvfile, graph_name, col_graph, elapsed, itrs,
                                        str(len(col_graph.getAvailable())), NAME[CODE['AP3']],
            ...
            with open(statsfile, 'a', newline='') as csvfile:
                ...
                if isgammanashboolean:
                    ...
                    while itrs < UPPERBOUND_ITERATIONS and isgammanashboolean == False:
                        ...
                        auxiliary.dumpMultiData(csvfile, graph_name, col_graph, elapsed, itrs, str(KOLORS),
                                                NAME[CODE['LLL']],
                                                NULL_EPSILON, avgdeg, maxdeg, isnashboolean, isgammanashboolean,
                                                gammavalue,
                                                fractionvalue_nash, fractionvalue_gamma_nash)
                        itrs += 1
                    bar.finish()
